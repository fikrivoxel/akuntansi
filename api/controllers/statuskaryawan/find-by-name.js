module.exports = {
  ...
  description: 'Find Status Karyawan by name',
  ...
  fn: async function (inputs, exits) {
    const total = await StatusKaryawan.count({
      nama_status: {
        contains: inputs.nama_status
      }
    })
    .intercept((err) => {
    ...
    const data = await StatusKaryawan.find({
      nama_status: {
        contains: inputs.nama_status
      }
    })
    ...
    return exits.notFound({
      status: 'failed',
      message: `Status Karyawan with name ${inputs.nama_status} not found`
    })
  }
