module.exports = {
  ...
  description: 'Find StokFisik by name',
  ...
  fn: async function (inputs, exits) {
    const total = await StokFisik.count({
      stok: {
        '>=': inputs.greater,
        '<=': inputs.later
      }
    ...
    const data = await StokFisik.find({
      stok: {
        '>=': inputs.greater,
        '<=': inputs.later
      }
    ...
    return exits.notFound({
      status: 'failed',
      message: `Stok fisik with stok later or equal to ${inputs.later} and greater or equal to ${inputs.greater} not found`
    })
  }
