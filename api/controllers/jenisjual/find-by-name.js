module.exports = {
  ...
  description: 'Find Jenis Jual by name',
  ...
  fn: async function (inputs, exits) {
    const data = await JenisJual.find({
      jenis_jual: {
        contains: inputs.jenis_jual
      }
    })
    ...
    const total = await JenisJual.count({
      jenis_jual: {
        contains: inputs.jenis_jual
      }
    })
    ...
    return exits.notFound({
      status: 'failed',
      message: `Jenis Jual with name ${inputs.jenis_jual} not found`
    })
  }
