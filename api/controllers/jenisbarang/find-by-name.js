module.exports = {
  ...
  fn: async function (inputs, exits) {
    const total = await JenisBarang.count({
      jenis_barang: {
        contains: inputs.jenis_barang
      }
    })

    const data = await JenisBarang.find({
      jenis_barang: {
        contains: inputs.jenis_barang
      }
    })
    ...
    return exits.notFound({
      status: 'failed',
      message: `Jenis barang with name ${inputs.jenis_barang} not found`
    })
  }
